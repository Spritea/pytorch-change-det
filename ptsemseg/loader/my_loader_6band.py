...
class myLoader_6band(data.Dataset):
    def __init__(
            self,
            root,
            split="train",
    ...
    def __getitem__(self, index):
        ...
        #tensor img_src shape:[3,512,512]
        img_6band=torch.cat([img_src,img_dst],dim=0)
        if self.split=='test':
            return img_path_src,img_6band
        else:
            return img_6band, lbl
