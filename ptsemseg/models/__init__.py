...
from ptsemseg.models.fc_ef import fc_ef
from ptsemseg.models.fc_siam_conc import fc_siam_conc
from ptsemseg.models.fc_siam_diff import fc_siam_diff
from ptsemseg.models.cdnet import cdnet
from ptsemseg.models.changenet import changenet50
...
def get_model(model_dict, n_classes):
    ...
    #     model=model(num_classes=n_classes)
        ...
        model=model(num_classes=n_classes,**param_dict)
    elif name=="fc_siam_conc":
        model=model(num_classes=n_classes,**param_dict)
    elif name=="fc_siam_diff":
        model=model(num_classes=n_classes,**param_dict)
    elif name=="cdnet":
        model=model(num_classes=n_classes,**param_dict)
    elif name == "changenet50":
        model = model(num_classes=n_classes, **param_dict)
