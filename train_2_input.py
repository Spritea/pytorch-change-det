...
def train(cfg, writer, logger,run_id):
    ...
    while i <= train_iter and flag:
        for (image_src,images_dst, labels) in trainloader:
            i += 1
            start_ts = time.time()
            scheduler.step()
            model.train()
            image_src = image_src.to(device)
            image_dst=images_dst.to(device)
            ...
            if (i + 1) % cfg['training']['val_interval'] == 0 or \
               (i + 1) == train_iter:
                model.eval()
                with torch.no_grad():
                    for i_val, (image_src_val,image_dst_val, labels_val) in tqdm(enumerate(valloader)):
                        image_src_val = image_src_val.to(device)
                        image_dst_val = image_dst_val.to(device)
                        labels_val = labels_val.to(device)

                        outputs = model(image_src_val,image_dst_val)
